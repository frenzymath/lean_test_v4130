...
def mkRelatedTheorem (id: Nat) (thm : Json) : String :=
  "ID: " ++ toString id ++ "\n" ++
  "Formal name: " ++ toString (Json.getObjValD thm "Formal name") ++
  "Informal name: " ++ toString (Json.getObjValD thm "Informal name") ++
  "Formal statement: " ++ toString (Json.getObjValD thm "Formal statement")
...
def makePrompt (tacticState : String) (relatedTheorems: Array Json) : String :=
  "User: Please generate a tactic in lean4 to solve the state.
Here're some theorems that may be helpful:\n" ++
"\n".joinSep (Array.mapIdx' mkRelatedTheorem relatedTheorems) ++
"\nSTATE" ++ tacticState ++ "\nTACTIC:\n\n\nAssistant:"


def API.tacticGeneration
  (api : API) (tacticState : String) (relatedTheorems: Array Json) :
  IO $ Array (String × Float) := do
  let prompt := makePrompt tacticState relatedTheorems
  let options := GenerationOptions.mk 0.9 8
  tacticGenerationOpenAI "" [prompt] api options


/- Calls an LLM API with the given context, prefix and pretty-printed goal. -/
def runSuggest (goal: String) : IO (Array (String × Float)) := do
  let api ← getAPI
  let relatedTheorems: Array Json := (sorry)
  let s ← api.tacticGeneration goal relatedTheorems
  return s
